...
static void
emu_riscv_set_pc(emu_t* emu, const uint32_t value)
{
...
emu_riscv_load_elf(emu_t* emu, const target_t* target)
{
    ...
    emu_riscv_set_reg(emu, RISC_V_REG_PC, target->elf->entry_point);
...
emu_riscv_build_stack(emu_t* emu, const target_t* target)
{
    ...
    emu->registers[RISC_V_REG_SP] = stack_start + emu->stack_size;
...
static void
emu_riscv_setup(emu_t* emu, const target_t* target)
{
    emu_riscv_load_elf(emu, target);
    emu_riscv_build_stack(emu, target);
}
