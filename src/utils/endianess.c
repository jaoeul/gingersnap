...
uint64_t
byte_arr_to_u64(uint8_t* bytes, size_t size, enum_endianess_t endianess)
{
    return (endianess == ENUM_ENDIANESS_LSB) ? lsb_byte_arr_to_u64(bytes, size) : msb_byte_arr_to_u64(bytes, size);
}
...
{
    return endianess == ENUM_ENDIANESS_LSB ? u64_to_byte_arr_lsb(num, result_bytes) : u64_to_byte_arr_msb(num, result_bytes);
}
